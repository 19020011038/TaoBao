...
public class MyDataBaseHelper extends SQLiteOpenHelper {
    ...
    public MyDataBaseHelper(@Nullable Context context) {
        ...
        super(context, "database", null, 20);
    }

    @Override
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        //下面这句是简单的SQL语句,创建数据库的表
        String TABLE_USER = "create table user(id_user INTEGER PRIMARY KEY autoincrement,number text,nickname text,introduction text,password text,location text,picture blob);";
        sqLiteDatabase.execSQL(TABLE_USER);
        String TABLE_SHOP = "create table shop(id_shop INTEGER PRIMARY KEY autoincrement,id_user text,shop_name text,picture blob);";
        sqLiteDatabase.execSQL(TABLE_SHOP);
        String TABLE_COMMODITY = "create table commodity(id_commodity INTEGER PRIMARY KEY autoincrement,id_user text,id_shop text,commodity_name text,price text,number text,picture blob);";
